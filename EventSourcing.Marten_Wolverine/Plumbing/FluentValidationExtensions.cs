...

{
    ...
    //     });
    // }   
    public static IRuleBuilderOptions<T, TU> MustBeValueObject<T, TValueObject, TU>(this IRuleBuilder<T, TU> ruleBuilder, Func<TU, TypeResult<TValueObject>> factory)
    {
        return (IRuleBuilderOptions<T, TU>)ruleBuilder.Custom((s, context) =>
        {
            try
            {
                factory(s).MatchFailure(error => context.AddFailure(error.Error.Message), () => {});
            }
            catch (Exception e)
            {
                context.AddFailure(e.Message);
